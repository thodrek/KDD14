...
tags each article with a (lat, long) information and a date. From the (lat, long) information, 
we infer the state and country of the article. Using some in-built modules in python, we
also infer the source and/or publisher of the article. Thus for each article we can extract the following information:
word count, location, source and timestamp. 

*Talk to theo and take some suggestions*
...
All models are implemented in Python and the evaluation is performed on an x (machine information to be added) machine
\subsection{GSR}
...
Figure x shows the keyword cloud for 12 disease topics discovered from the Healthmap corpus.
...
"hantavirus", "lluvia" (rainfall) and "corriente" (water current). These keywords make 
sense as Hantavirus is mainly a cardiopulmonar disease caused by infection of rodents and/or rats 
in mainly rural areas. The population of rodents is influenced by Rainfall and 
thus increase in rainfall or water current is an indirect indicator of Hantavirus outbreaks.
Figure x shows the evolution of Topic 2 at different points of the timeline. It 
clearly depicts the fact that the hantavirus topics are becoming increasingly 
prominent as we train the Topic Model with more incoming articles along the timeline.

In figure x, we study the focus of the Hantavirus topics on different locations.
Under each of the three topics of interest, we find the higher ranked locations, 
i.e. locations in which the rare disease topics are more prominent. In the figure we see that 
the rare disease topics are mostly clustered in the regions of Chile and Argentina. 
Pie chart in figure x demonstrates that these two countries are most vulnerable to Hantavirus 
outbreaks which validates the above outcome.  
