...
$w \in V$. To compute the expected frequency for $w$, we need to consider the 
probability of source $s$ publishing at a future timepoint $t$ mentioning the word $w$, denoted by $\Pr(t|s)$ unique to that word $w$, the 
probability of source $s$ publishing in the location $l$ mentioning the word $w$, denoted by $\Pr(l|s)$ unique to that word $w$ and 
the probability that word $w$ was generated by any topic $z \in K$, given location $l$ and time point $t$. 
More precisely, we have that:
\begin{equation}
  \hat{F}_{s,l,t}[w] = \bar{x}_{w} \cdot \Pr(t|s) \cdot \Pr(l|s)\cdot \sum_{z \in K}\phi_{z,w}\cdot \theta_{l,z} \cdot \xi_{z,t}
\end{equation}
...
and $\Pr(l|s)$ for a word $w$ can be written as:
\begin{equation}
  \Pr(l|s) = \frac{n_{w,s,l}}{\sum_{w\in V}n_{w,s,l}}
\end{equation}
where, $n_{w,s,l}$ denotes the number of mentions of $w$ from $s$ in $l$.
Notice that $\xi_{z,t}$, i.e., the probability of topic $z$ being prominent at time $t$, 
and $\Pr(t|s)$ correspond to future time points and need to be estimated.  
...
publishing  at a future time $t$ mentioning a particular word $w$ by considering the weighted average rate of the source mentioning that word:
\begin{equation}
\Pr(t|s) = \frac{\sum_{\tau = 1}^{t-1} \frac{1}{t - \tau}I(\tau,s)}{\sum_{\tau = 1}^{t-1} \frac{1}{t - \tau}}
\end{equation}
where $I(s,\tau)$ is an indicator variable equal to one if the source $s$ mentioned the word $w$ in an article at the time $\tau$ and zero otherwise. 
