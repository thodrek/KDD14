...
\label{sec:exp}
1. Give an introduction to this section. Discuss about the comparison of two models (location-based and source-based model),

2. State the length of our study on detecting Hantavirus outbreaks from January 2013 to March 2014, i.e. we are comparing both the models over this time line.
\subsection{Input Setup}
1. Give an extensive description of the Healthmap Dataset we used for training both the models.
2. Give the timeline for which this dataset is used to train both of our models, i.e. from June 2012 to March 2014.
3. write a few sentences about the pre-processing steps such as stop-word removal and removal of other unimportant words.
4. write about the initial training of the tensor (from June 2012 to October 2012) and then weekly training of the tensor starting from October 2012 to March 2014. Also write few sentences about the weekly topic distribution outputs and how the prediction model is run weekly on these topic distributions to generate the cosine similarity values for both the models.
5. Give a detailed numerical description of the tensor (number of words, number of locations, number of sources)
 
\subsection{GSR}

1. In this section we will discuss in details about preparation of the GSR ground truth data, how the analysts search through multiple News sources as well as PROMED to determine confirmed hantavirus events. 
2. Also state how an event is described such as (event date, first reported date, the first news source reporting the event, etc.)
3. Also discuss the 6-month rule.
...
1. Discuss in details about the different parameter settings used in different parts of the model such as parameter settings in topic modeling part, also write few sentences why parameters are set to these values.

2. Discuss about the cross validation procedure in one-class SVM to select the parameters of one-class SVM unique to each location and source.

3. Discuss about different parameters used in multiplicative weights.
